/*
 * TransitionQuaternionEigen.hh
 */

#ifndef TRANSITIONQUATERNIONEIGEN_HH
#define TRANSITIONQUATERNIONEIGEN_HH
...
public:
  ...
  Quaternion transition( const int _i) const         { if(_i >= 0) return transitions_[_i]; else return inverse_transition(-_i);}

  // get the quaternion of the inverse transition
  Quaternion inverse_transition( const int _i) const { if(_i >= 0) return transition( inverse_transition_idx(_i)); else return transition(-_i); }
  ...
  // return the index of the transition t s.t. _i*t is_2d_rotation and t closest to _q
  Quaternion closest_transition_generating_2d( const Quaternion& _q, const int _i) const { return transition( closest_transition_idx_generating_2d(_q, _i)); }
  ...
  int rotation_axis_2d( const int _i) const { if(_i >= 0) return rotation_axis_2d_[_i]; else return rotation_axis_2d(-_i); }

  int rotation_angle_2d( const int _i) const { if(_i >= 0) return rotation_angle_deg_2d_[_i]; else return rotation_angle_2d(-_i); }
...
#endif /* TRANSITIONQUATERNIONEIGEN_HH */
