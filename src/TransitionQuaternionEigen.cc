...
init_transition_quaternions()
{
  ...
  // 2: rotations around {x,y,z} by 180�
  ...
  // 3: rotations around {x,-x,y,-y,z,-z} by 90�
  ...
  // 4: rotations around {(x,y), (y,z), (z,x) (y,-x), (z,-y), (x,-z)} by 180�
...
test_round_to_2d_possibilities()
{
  ...
  for(int i=0; i<24; ++i)
    ...
    {
      int c;
      std::vector<int> iv; iv.push_back(i); iv.push_back(j);
  ...
  for(int i=0; i<24; ++i)
    for(int j=0; j<24; ++j)
      ...
      {
	int c;
	std::vector<int> iv; iv.push_back(i); iv.push_back(j); iv.push_back(k);
	if( !find_closest_transition_idx_generating_2d( Quaternion(1,0,0,0), iv, c))
	{
